...
func main() {
	buildModifiesField("R:\\Thales\\Game\\SteamLibrary\\steamapps\\common\\Crusader Kings II")

	allModifier := modifier.LoadAllModifier("R:\\Thales\\Game\\SteamLibrary\\steamapps\\common\\Crusader Kings II")
...
func buildModifiesField(dir string) {
	modifiers_dir := filepath.Join(dir, "common", "event_modifiers")
	// 读取目录中的所有文件和子目录
	files, err := os.ReadDir(modifiers_dir)
	if err != nil {
	...
	utils.WriteContent("logs/modifies_fields.txt", boolFieldsContent+stringFieldsContent+float32FieldsContent+intFieldsContent+structFieldsContent)
}
